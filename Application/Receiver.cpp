...
void WebClient::Request(IO::Client& c, const std::string& response, bool gzip) {
	...
	if (r == "/") {
		Response(c, "text/html", (char*)i2_html_gz, i2_html_gz_len, true);
	}
	if (r == "/i1") {
		Response(c, "text/html", (char*)i1_html_gz, i1_html_gz_len, true);
	}
	if (r == "/i2") {
	...
	else if (r == "/ships_mini_geo.json") {
		std::string content = ships.getGEOJSONmini();
		Response(c, "application/json", content, use_zlib & gzip);
	}
	
	else if (r == "/ship2json") {
		std::string content = ships.getship2JSON();
		Response(c, "application/json", content, use_zlib & gzip);
	}

	else if (r == "/ship5json") {
		std::string content = ships.getJSONfive();
		Response(c, "application/json", content, use_zlib & gzip);
	}
	...
	// added by 230618		
		...
		std::string content = ships.getJSON6SHIP_M();
		Response(c, "application/json", content, use_zlib & gzip);
	}	
	else if (r == "/ship6a") {
		std::string content = ships.getJSON5SHIP_A();
		Response(c, "application/json", content, use_zlib & gzip);
	}	

	else if (r == "/station") {
		std::string content = ships.getJSON_STATION();
		Response(c, "application/json", content, use_zlib & gzip);
	}	

	else if (r == "/aton") {
		std::string content = ships.getJSON_ATON();
		Response(c, "application/json", content, use_zlib & gzip);
	}

	else if (r == "/sar") {
		std::string content = ships.getJSON_SAR();
		Response(c, "application/json", content, use_zlib & gzip);
	}

	else if (r == "/sg") {
		std::string content = ships.getJSON_SG();
		Response(c, "application/json", content, use_zlib & gzip);
	}

	else if (r == "/sart") {
		std::string content = ships.getJSON_AIS_SART();
		Response(c, "application/json", content, use_zlib & gzip);
	}

	else if (r == "/uso") {
		std::string content = ships.getJSON_USO();
		Response(c, "application/json", content, use_zlib & gzip);
	}
	// end 230618
	...
	// add 230624
		...
		std::string content = ships.getJSON_ship7();
		Response(c, "application/json", content, use_zlib & gzip);
	}		
