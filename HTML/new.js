<script>
    const initialCenter = [25, 44],
        initialZoomLevel = 3,
        map = L.map("map").setView(initialCenter, initialZoomLevel),
        commonOptions = { key: "nRYox0R1ZyZ6XqSStq4S", accessToken: "qML7E6HmMKb6LQJgxHedkuht58y48dIpzawFGfCXdHzqnZWQlscx5zmyw7uYgTZG" },
        LLP = L.tileLayer.provider,
        tileLayers = {
    ...
    tileLayers["Stadia AlidadeSmoothDark"].addTo(map), map.removeControl(map.zoomControl), L.control.layers(tileLayers, null, { position: "bottomright" }).addTo(map), L.control.zoom({ position: "bottomright" }).addTo(map);
    const markerClusterGroup = L.markerClusterGroup({ disableClusteringAtZoom: 12 }),
        shipColors = {
            1: "#CD5C5C",
            2: "#F08080",
            3: "#FA8072",
        ...
        shipIconSvg =
            '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 6 12"><path d="{{SHIP_ICON_PATH}}" style="fill:{{SHIP_TYPE_COLOR}};fill-opacity:.8;stroke:#010101;stroke-width:.15;stroke-dasharray:none;stroke-opacity:.8"/></svg>',
        shipIconUrl = "data:image/svg+xml;charset=UTF-8," + encodeURIComponent(shipIconSvg),
        AToNIconUrl = "data:image/svg+xml;charset=UTF-8," + encodeURIComponent(AToNIconSvg),
        SARIconUrl = "data:image/svg+xml;charset=UTF-8," + encodeURIComponent(SARIconSvg),
        bstIconUrl = "data:image/svg+xml;charset=UTF-8," + encodeURIComponent(bstIconSvg),
        UFOIconUrl = "data:image/svg+xml;charset=UTF-8," + encodeURIComponent(UFOIconSvg);
    for (let e = 0; e <= 16636; e++) shipColors.hasOwnProperty(e) || (shipColors[e] = "red");
    const markerIcons = {
        ship: L.divIcon({
            className: "ship-icon",
            iconSize: [24, 24],
    ...
    let countControl,
        ufoLayerGroup = L.layerGroup();
    async function createMarkers() {
        try {
            const l = await fetch("/ship5json"),
                t = await l.json();
            let n = 0,
                s = 0,
                o = 0,
            ...
            (n = t.length.toLocaleString()),
                ...
                t.forEach((e) => {
                    const [t, l, d, m, f] = e;
                    let u, h, n;
                    if (t >= 201e6 && t <= 775999999) {
                        (u = "Ship"), (h = markerIcons.ship);
                        const e = shipColors[parseInt(f)] || "gray";
                        if (m === null) {
                            c++;
                            const s = L.circleMarker([l, d], { radius: 4, color: e, fillOpacity: 0.9 });
                            n = s;
                            const o = `MMSI: ${t}<br>Shiptype: ${f} <br>Position: ${[l, d]}`;
                            s.bindTooltip(o).openTooltip();
                        } else {
                            r++;
                            const s = "m 0 12 l 3 -12 l 3 12 c -2.25 -2 -3.75 -2 -6 0 z",
                                o = shipIconSvg.replace(/{{SHIP_TYPE_COLOR}}/g, e).replace(/{{SHIP_ICON_PATH}}/g, s),
                                i = "data:image/svg+xml;charset=UTF-8," + encodeURIComponent(o),
                                a = L.icon({ iconUrl: i, iconSize: [12, 12] }),
                                c = `transform: rotate(${m}deg);`,
                                u = `<img src="${a.options.iconUrl}" style="${c}">`;
                            n = L.marker([l, d], { icon: L.divIcon({ className: "ship-icon", html: u, iconSize: [12, 12], iconAnchor: [6, 6] }), rotationAngle: m });
                            const h = `MMSI: ${t}<br>Heading: ${m}&deg<br>Shiptype: ${f} <br>Position: ${[l, d]}`;
                            n.bindTooltip(h).openTooltip();
                        }
                    } else if (t >= 201e4 && t <= 7759999) {
                        (u = "Base station"), (h = markerIcons.baseStation), (n = L.marker([l, d], { icon: h }));
                        const e = `MMSI: ${t}<br>Type: ${u}`;
                        n.bindTooltip(e), i++;
                    } else if (t >= 111201e3 && t <= 111775999) {
                        (u = "SAR"), (h = markerIcons.SAR);
                        const e = [32, 32];
                        n = L.marker([l, d], { icon: L.icon({ iconUrl: h.options.iconUrl, iconSize: e }) });
                        const s = `MMSI: ${t}<br>Type: ${u}`;
                        n.bindTooltip(s), o++;
                    } else if (t >= 99201e4 && t <= 997759999) {
                        (u = "AtoN"), (h = markerIcons.AtoN), (n = L.marker([l, d], { icon: h }));
                        const e = `MMSI: ${t}<br>Type: ${u}<br>Position: ${[l, d]}`;
                        n.bindTooltip(e), s++;
                    } else {
                        (u = "UFO"), (h = markerIcons.UFO), (n = L.marker([l, d], { icon: h }));
                        const e = `MMSI: ${t}<br>Type: ${u}<br>Shiptype: ${f}`;
                        n.bindTooltip(e), a++;
                    }
                    markerClusterGroup.addLayer(n);
                }),
                countControl && countControl.remove(),
                (countControl = L.control({ position: "topleft" })),
                (countControl.onAdd = function () {
                    const t = L.DomUtil.create("div", "ship-count-box");
                    return (t.innerHTML = `<span id="hpradar-link">© HPRadar</span> Navitrack|📡${n}|🚢${r.toLocaleString()}|⚓${c.toLocaleString()}|🗼${i}|🚁${o}|🛟${s.toLocaleString()}|🛸${a}`), t;
                }),
                countControl.addTo(map);
            const e = document.getElementById("hpradar-link");
            e.addEventListener("mouseover", function () {
                (e.style.cursor = "pointer"), (e.style.textDecoration = "underline");
            }),
                e.addEventListener("mouseout", function () {
                    (e.style.cursor = "default"), (e.style.textDecoration = "none");
                }),
    ...
    createMarkers(), setInterval(createMarkers, 20 * 1e3);
</script>
