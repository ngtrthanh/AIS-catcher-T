...
Device::Device* Receiver::getDeviceByType(Type type) {
    static std::map<Type, Device::Device*> deviceMap = {
        {Type::WAVFILE, &_WAV},
        {Type::RAWFILE, &_RAW},
        {Type::RTLTCP, &_RTLTCP},
        {Type::SPYSERVER, &_SpyServer},
        {Type::UDP, &_UDP},
#ifdef HASZMQ
        {Type::ZMQ, &_ZMQ},
#endif
#ifdef HASAIRSPYHF
        {Type::AIRSPYHF, &_AIRSPYHF},
#endif
#ifdef HASAIRSPY
        {Type::AIRSPY, &_AIRSPY},
#endif
#ifdef HASSDRPLAY
        {Type::SDRPLAY, &_SDRPLAY},
#endif
#ifdef HASRTLSDR
        {Type::RTLSDR, &_RTLSDR},
#endif
...
void Receiver::setupDevice() {
    ...
    if (!serial.empty() || type != Type::NONE) {
        for (int i = 0; i < device_list.size(); i++) {
            if ((device_list[i].getSerial() == serial && (type == Type::NONE || type == device_list[i].getType())) ||
                (serial.empty() && type == device_list[i].getType())) {
                idx = i;
    ...
    if (type == Type::NONE && device_list.empty()) {
        throw std::runtime_error("No devices available");
    }

    device = (type != Type::NONE) ? getDeviceByType(type) : getDeviceByType(device_list[idx].getType());
    ...
    device->setFrequency((ChannelMode == AIS::Mode::AB) ? 162000000 : 156800000);
...
std::unique_ptr<AIS::Model>& Receiver::addModel(int m) {
    static const std::vector<std::function<std::unique_ptr<AIS::Model>()>> modelFactories = {
        []() { return std::make_unique<AIS::ModelStandard>(); },
        []() { return std::make_unique<AIS::ModelBase>(); },
        []() { return std::make_unique<AIS::ModelDefault>(); },
        []() { return std::make_unique<AIS::ModelDiscriminator>(); },
        []() { return std::make_unique<AIS::ModelChallenger>(); },
        []() { return std::make_unique<AIS::ModelNMEA>(); }
    };

    if (m >= 0 && m < modelFactories.size()) {
        models.push_back(modelFactories[m]());
    } else {
...
void Receiver::setupModel() {
    ...
    // ensure compatibility between model and device
    AIS::ModelClass expectedClass = (device->getFormat() == Format::TXT) ? AIS::ModelClass::TXT : AIS::ModelClass::NMEA;
    for (const auto& m : models) {
        if (m->getClass() != expectedClass)
...
void Receiver::play() {
    ...
    if (verbose) {
        ...
        for (const auto& model : models)
            std::cerr << "Model #" << std::to_string(&model - &models[0]) << "  : [" << model->getName() << "] " << model->Get() << "\n";
    }
}
...
void WebClient::Request(IO::Client& c, const std::string& response, bool gzip) {
	...
	else if (r == "/ships_array_geo.json") {
		std::string content = ships.getGEOJSONcompact();
		Response(c, "application/json", content, use_zlib & gzip);
	}
	
	else if (r == "/ships_tiny_geo.json") {
		std::string content = ships.getGEOJSONtiny();
		Response(c, "application/json", content, use_zlib & gzip);
	}	
	else if (r == "/ships_tiny") {
		std::string content = ships.getGEOJSONtiny();
		// Debug code: Log the JSON content to Docker logs
		std::cerr << "Generated JSON content: " << content << std::endl;	
		Response(c, "application/json", content, use_zlib & gzip);
	}
